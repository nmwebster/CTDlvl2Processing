...
'sigma-t11: Density, 2 [sigma-t, kg/m^3 ]                     Density2_[kg/m3_sigmat]          ';...
'sigma-é00: Density [sigma-theta, kg/m^3]                     Density_[kg/m3_sigmat]           ';...
'sigma-é11: Density, 2 [sigma-theta, kg/m^3]                  Density2_[kg/m3_sigmat]          ';...
...
%        if ~stnname, wu = findstr(FILEtitle,'-'); STNNAME = upper(FILEtitle(wu(end)+1:end-4)); end
   ...
   if extrapolateFROM > 0,
       ...
       % CSN = INfile; % temporarily assign a consecutive station number
        ...
        % Creating Array of Variable CSV HEADERS for profile/bottle csv
        % summary files
        outfmtH = '';
        warning('off','MATLAB:dispatcher:InexactCaseMatch')
        for in = 1:m,
            VARrow = strmatch(strtrim(COLUMNnames(in,:)), VARIABLEnameFORMATheader);
            if ~isempty(VARrow),
                outfmtH = [outfmtH strtrim(VARIABLEnameFORMATheader(VARrow,62:end)) ','];  % change the 62 if the variable definition array got bigger
            else
                disp(['No corresponding column found for: ' strtrim(COLUMNnames(in,:))])
                keyboard
            end
