...
dlgPrompts={['Input Depth for New Salinity:'],['Input Salinity Value at Depth:']};
dlgTitle='Input new depth and Salinity:';
dlgLines=1;
dlgDefaults={['0'],['32']};
dlgInput=inputdlg(dlgPrompts,dlgTitle,dlgLines,dlgDefaults);

newDep = str2num(dlgInput{1});
newSal = str2num(dlgInput{2});
  ...
  if ~isempty(newSal)	
	if newSal < 0,
		display(['Salinity must be in range 0 to 40']);
		oksofar = 0;
  	elseif newSal > 40,
		display(['Salinity must be in range 0 to 40']);
		oksofar = 0;
